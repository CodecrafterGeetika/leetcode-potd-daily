public class Solution { 
    ...
    private int[] fact; 
    private int[] invFact; 
    private int binaryExp(long a, long b) { long res = 1; 
                                           while (b > 0) { 
                                               if ((b & 1) == 1) { 
                                                   res = (res * a) % MOD;
                                               }
    ...
    private void inverseFact(int n) { invFact = new int[n + 1]; 
        ...
        fact = new int[n + 1]; 
                                                                     fact[0] = 1; 
        for (int i = 1; i <= n; i++) { fact[i] = (int)((1L * i * fact[i - 1]) % MOD); 
                                     } 
    }
    private void precompute(int n) { factorial(n);
                                    inverseFact(n); 
                                   } 
